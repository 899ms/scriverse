...
After the administrator exists, change `.env` to fully disable registration, or switch to invite-only registration:
...
`APP_ALLOW_REGISTRATION` is three mutually exclusive levels: unset, `false`, `0`, or any other value disables registration; `invite` requires a one-time invite code generated by a system administrator; `true` or `1` opens registration. Invite-only and open registration both require an `APP_SETUP_TOKEN` of at least 32 characters. Unset and all other values disable both the UI and backend registration endpoint, including first-administrator setup on an empty database. The setup token is checked only when creating the first administrator. Invite codes are consumed only for later registrations after that administrator exists.
...
Set `APP_ALLOW_REGISTRATION=true` or `invite` and an `APP_SETUP_TOKEN` of at least 32 characters for first-time setup, then recreate the container. After creating the administrator, set the switch to `false` to close registration, or to `invite` so later users must present a one-time invite code. System administrators generate invite codes in user management; each code can be used once.

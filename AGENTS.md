...
- 测试完成后必须关闭本地测试版本 APP 与测试网页标签；只有用户明确要求保留或不关闭时才可例外。
...
- 作品侧栏 `#module-nav` 中，「更多 / 收起」之前只保留既有主模块入口。新增导航 icon、对话框入口或次要模块按钮，默认必须放进 `#module-more-button` 控制的 `.module-nav-secondary` 折叠区。除非用户明确要求放到折叠外面，否则禁止把新入口加到折叠区外。

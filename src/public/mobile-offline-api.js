export class MobileOfflineApiError extends Error {
  constructor(code, message) {
    super(message);
    this.name = "MobileOfflineApiError";
    this.code = code;
  }
}
...
function snapshotRecord(entity) {
  ...
  return {
    ...
    mobileLocalRevisionNo: Number(entity.localRevisionNo ?? 0),
    mobileSyncConflict: entity.conflict === true,
    mobileReadOnly: entity.locked === true
  };
}
...
export class MobileOfflineApi {
  ...
  async work(workId, { includeVolumes = false } = {}) {
    ...
    if (!cached) throw new MobileOfflineApiError("SYNC_WORK_NOT_FOUND", "离线副本中不存在该作品");
  ...
  async volumeChapters(volumeId, url) {
    ...
    throw new MobileOfflineApiError("SYNC_VOLUME_NOT_FOUND", "离线副本中不存在该分卷");
  }
  ...
  async entity(entityType, entityId) {
    ...
    throw new MobileOfflineApiError("SYNC_ENTITY_NOT_FOUND", "离线副本中不存在该记录");
  }
  ...
  async saveEntity(entityType, entityId, body) {
    ...
    if (current.entity.locked || current.entity.conflict) {
      throw new MobileOfflineApiError("SYNC_ENTITY_READ_ONLY", "该记录存在冲突或已锁定为只读，请先在同步中心处理");
    }
    ...
    await this.client?.emitStatus("saved", current.workId);
    return {
      ...snapshot,
      versionNo: Number(current.entity.serverVersionNo),
      mobileLocalRevisionNo: Number(saved.localRevisionNo),
      updatedAt: saved.savedAt
    };
  }

  unsupported() {
    throw new MobileOfflineApiError(
      "MOBILE_OFFLINE_OPERATION_UNSUPPORTED",
  ...
  async request(path, options = {}) {
    ...
    const url = new URL(path, globalThis.location?.origin ?? "https://mobile.invalid");
...
export function createMobileOfflineApi(options) {
  return new MobileOfflineApi(options);
}

...
export class MobileSyncStoreError extends Error {
  constructor(code, message) {
    super(message);
    this.name = "MobileSyncStoreError";
    this.code = code;
  }
}
function assertUuid(value, label) {
  if (typeof value !== "string" || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(value)) {
    throw new MobileSyncStoreError("SYNC_ID_INVALID", `${label} 无效`);
  }
  return value;
}

function assertEntityType(value) {
  if (!EDITABLE_ENTITY_TYPES.has(value)) throw new MobileSyncStoreError("SYNC_ENTITY_TYPE_UNSUPPORTED", "该类型暂不支持离线编辑");
  return value;
}
...
export function mobileSyncDatabaseName(profileId, userId) {
  return `scriverse-mobile-sync-v1-${assertUuid(profileId, "profile id")}-${assertUuid(userId, "user id")}`;
}
...
function openDatabase(indexedDBImpl, name) {
  return new Promise((resolve, reject) => {
    ...
    request.addEventListener("blocked", () => reject(new MobileSyncStoreError("SYNC_DATABASE_BLOCKED", "离线数据库升级被其他窗口阻止")), { once: true });
  });
}
...
export class MobileSyncStore {
  constructor({ profileId, userId, indexedDBImpl = globalThis.indexedDB, cryptoImpl = globalThis.crypto }) {
    ...
    if (!indexedDBImpl) throw new MobileSyncStoreError("SYNC_DATABASE_UNAVAILABLE", "当前环境无法使用离线功能");
  ...
  async open() {
    if (!this.databasePromise) {
      this.databasePromise = openDatabase(this.indexedDB, mobileSyncDatabaseName(this.profileId, this.userId))
        .then(async (database) => {
          ...
          if (existing && (
            ...
            transaction.abort();
            throw new MobileSyncStoreError("SYNC_DATABASE_IDENTITY_MISMATCH", "本机副本与当前工作区不匹配");
          }
          if (!existing) {
            store.put({
              key: "identity",
  ...
  async setInitialOfflineDownloadState({ decision, completed }) {
    if (decision !== "accepted" && decision !== "declined") {
      throw new MobileSyncStoreError("SYNC_INITIAL_DOWNLOAD_STATE_INVALID", "首次离线下载选择无效");
    }
    if (typeof completed !== "boolean" || (decision === "declined" && completed !== true)) {
      throw new MobileSyncStoreError("SYNC_INITIAL_DOWNLOAD_STATE_INVALID", "首次离线下载完成状态无效");
    }
    const value = {
  ...
  async replaceSnapshot({ workId, cutoffCursor, items, permissionsSnapshot = null }) {
    if (typeof workId !== "string" || workId.length === 0 || !Array.isArray(items)) {
      throw new MobileSyncStoreError("SYNC_SNAPSHOT_INVALID", "同步快照无效");
    }
    ...
    if (currentStatus.pending > 0 || currentStatus.syncing > 0 || currentStatus.conflicts > 0 || currentStatus.rejected > 0) {
      throw new MobileSyncStoreError("SYNC_SNAPSHOT_DIRTY", "作品仍有未处理的本地变更，不能覆盖离线副本");
    }
    const workItem = items.find((item) => item.entityType === "work");
    if (!workItem?.data || String(workItem.entityId) !== workId) {
      throw new MobileSyncStoreError("SYNC_SNAPSHOT_INVALID", "同步快照缺少作品摘要");
    }
    const storedEntities = [];
    for (const item of items) {
      if (item.entityType === "work") continue;
      if (!item.data || typeof item.data !== "object") throw new MobileSyncStoreError("SYNC_SNAPSHOT_INVALID", "同步快照实体无效");
  ...
  async saveLocalEntity(workId, entityTypeValue, entityId, localSnapshot, changeNote = "手机端离线修改") {
    const entityType = assertEntityType(entityTypeValue);
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const current = await this.getEntity(workId, entityType, entityId);
      if (!current) throw new MobileSyncStoreError("SYNC_ENTITY_NOT_FOUND", "离线副本中不存在该记录");
      if (current.locked) throw new MobileSyncStoreError("SYNC_ENTITY_READ_ONLY", "该记录已因权限或同步失败锁定为只读");
      const database = await this.open();
      ...
      const mutation = {
        ...
        changeNote: String(changeNote).trim().slice(0, 500) || "手机端离线修改"
      };
    ...
    throw new MobileSyncStoreError("SYNC_LOCAL_SAVE_BUSY", "离线记录被其他窗口持续修改，请重试");
  }
  ...
  async listOutbox(workId, statuses = ["pending", "syncing", "conflict", "rejected"]) {
    if (!Array.isArray(statuses) || statuses.some((status) => !OUTBOX_STATUSES.has(status))) {
      throw new MobileSyncStoreError("SYNC_OUTBOX_STATUS_INVALID", "outbox 状态无效");
    }
    const database = await this.open();
  ...
  async setWorkStatus(workId, status, reason = null) {
    if (!["ready", "syncing", "offline", "read-only", "error"].includes(status)) {
      throw new MobileSyncStoreError("SYNC_WORK_STATUS_INVALID", "离线作品状态无效");
    }
    const database = await this.open();
    ...
    if (!work) {
      transaction.abort();
      throw new MobileSyncStoreError("SYNC_WORK_NOT_FOUND", "离线作品不存在");
    }
  ...
  async applyRemoteChanges(workId, changes, nextCursor) {
    if (!Array.isArray(changes) || !Number.isInteger(nextCursor) || nextCursor < 0) {
      throw new MobileSyncStoreError("SYNC_CHANGES_INVALID", "Server 增量变更无效");
    }
  ...
  async applyRemoteChange(workId, change) {
    if (
      ...
      || change.versionNo < 1
    ) throw new MobileSyncStoreError("SYNC_CHANGE_INVALID", "Server 增量变更字段无效");
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      if (!incomingSnapshot || typeof incomingSnapshot !== "object") {
        throw new MobileSyncStoreError("SYNC_CHANGE_INVALID", "Server upsert 缺少实体快照");
      }
      ...
      if (disposition === "conflict") {
        ...
        if (active.length === 0) throw new MobileSyncStoreError("SYNC_STORE_INVARIANT", "本地 dirty 实体缺少 outbox 记录");
    ...
    throw new MobileSyncStoreError("SYNC_LOCAL_SAVE_BUSY", "离线记录在拉取期间持续变化，请重试");
  }
  ...
  async applyPushResults(workId, results) {
    if (!Array.isArray(results)) throw new MobileSyncStoreError("SYNC_PUSH_RESULT_INVALID", "Server push 结果无效");
  ...
  async applyPushResult(workId, result) {
    if (!result || !OUTBOX_STATUSES.has(result.status === "applied" ? "syncing" : result.status)) {
      throw new MobileSyncStoreError("SYNC_PUSH_RESULT_INVALID", "Server push 结果字段无效");
    }
    const database = await this.open();
    ...
    if (result.status === "conflict") {
      if (!result.baseSnapshot || !result.localSnapshot || !result.serverSnapshot) {
        throw new MobileSyncStoreError("SYNC_PUSH_RESULT_INVALID", "Server 冲突结果缺少三方快照");
      }
      conflictRecord = {
    ...
    if (!entity || !currentOutbox) {
      transaction.abort();
      throw new MobileSyncStoreError("SYNC_STORE_INVARIANT", "push 结果找不到本地实体或 outbox");
    }
    if (result.status === "applied") {
      if (!serverSnapshot || !Number.isInteger(result.appliedVersionNo)) {
        transaction.abort();
        throw new MobileSyncStoreError("SYNC_PUSH_RESULT_INVALID", "Server applied 结果缺少权威快照");
      }
  ...
  async saveMergeDraft(mutationId, mergeDraft, unresolvedBlockCount) {
    ...
    await transactionDone(read);
    if (!conflict) throw new MobileSyncStoreError("SYNC_CONFLICT_NOT_FOUND", "同步冲突不存在");
    ...
    if (!locked) {
      transaction.abort();
      throw new MobileSyncStoreError("SYNC_CONFLICT_NOT_FOUND", "同步冲突不存在");
    }
    store.put({
      ...locked,
  ...
  async resolveConflict(mutationId, finalSnapshot, changeNote = "手机端冲突合并") {
    ...
    const mutation = {
      ...
      changeNote: String(changeNote).trim().slice(0, 500) || "手机端冲突合并"
    };
    const hash = await requestHash(mutation, this.crypto);
    const database = await this.open();
    const transaction = database.transaction(["entities", "outbox", "conflicts"], "readwrite");
    ...
    if (!entity) {
      transaction.abort();
      throw new MobileSyncStoreError("SYNC_ENTITY_NOT_FOUND", "冲突实体不存在");
    }
    entityStore.put({
      ...entity,
      localSnapshot: storedLocalSnapshot,
  ...
  async createRescueBundle(workId) {
    ...
    if (!work) throw new MobileSyncStoreError("SYNC_WORK_NOT_FOUND", "离线副本中不存在该作品");
    ...
    if (outbox.length === 0 && conflicts.length === 0) {
      throw new MobileSyncStoreError("SYNC_RESCUE_NOT_REQUIRED", "该作品没有需要导出的本机修改");
    }
    return {
      format: "scriverse-mobile-rescue",
      version: 1,

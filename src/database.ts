...
export class Database {
  ...
  private applyDataMigrations(): void {
    ...
    const registrationInviteTablePresent = this.get(
      "SELECT 1 AS present FROM sqlite_master WHERE type = 'table' AND name = 'registration_invites'"
    ) !== undefined;
    const registrationInviteIndexesPresent = [
    ...
    if (!applied.has(139) || !registrationInviteTablePresent || !registrationInviteIndexesPresent) {
      this.transaction(() => {
        this.run(`CREATE TABLE IF NOT EXISTS registration_invites (
          id TEXT PRIMARY KEY,
        ...
        this.run("CREATE INDEX IF NOT EXISTS idx_registration_invites_unused ON registration_invites(created_at) WHERE used_at IS NULL");
        this.run("CREATE INDEX IF NOT EXISTS idx_registration_invites_created ON registration_invites(created_at DESC, id)");
        this.run("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (139, ?)", new Date().toISOString());
      });
      const integrity = this.all<{ integrity_check: string }>("PRAGMA integrity_check");
      if (integrity.some((row) => row.integrity_check !== "ok")) {
        throw new Error(`数据库完整性检查失败：${integrity.map((row) => row.integrity_check).join("；")}`);
